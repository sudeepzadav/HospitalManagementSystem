...
const emptyAvailabilityRow = () => ({ day: "Monday", startTime: "09:00", endTime: "17:00" });
...
const DoctorsProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Something went wrong saving your profile. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: COLOR.paper, boxShadow: "0 30px 60px -20px rgba(28,43,38,0.25)" }}
      >
        <div className="flex items-center gap-2.5 mb-6">
          ...
          <span className="text-[15px] font-semibold tracking-tight" style={{ color: COLOR.ink }}>
        ...
        <h2
          ...
          style={{ fontFamily: "'Fraunces', serif", fontWeight: 500, color: COLOR.ink }}
        >
        ...
        <form onSubmit={handleSubmit} className="text-[14px]">
          {error && (
            <div
              ...
              style={{ color: COLOR.coral, background: "#FBEEEA", borderColor: "#F2D5CC" }}
            >
              {error}
            </div>
          )}

          <Field label="Department" icon={<Building2 size={17} style={{ color: COLOR.inkSoft }} />}>
            <select
              name="department"
          ...
          <Field label="Specialization" icon={<Stethoscope size={17} style={{ color: COLOR.inkSoft }} />}>
            <input
          ...
          <Field label="License number" icon={<BadgeCheck size={17} style={{ color: COLOR.inkSoft }} />}>
            <input
              className="w-full outline-none bg-transparent py-3 placeholder:text-[#9AA6A0]"
              style={{ color: COLOR.ink }}
              type="text"
          ...
          <Field label="Consultation fee ($)" icon={<DollarSign size={17} style={{ color: COLOR.inkSoft }} />}>
            <input
              className="w-full outline-none bg-transparent py-3 placeholder:text-[#9AA6A0]"
              style={{ color: COLOR.ink }}
              type="number"
          ...
          <Field label="Qualifications (comma separated)" icon={<GraduationCap size={17} style={{ color: COLOR.inkSoft }} />} noMargin>
            <input
              className="w-full outline-none bg-transparent py-3 placeholder:text-[#9AA6A0]"
              style={{ color: COLOR.ink }}
              type="text"
          ...
          <div className="mt-5 mb-2 flex items-center gap-2">
            ...
            <label
              ...
              style={{ color: COLOR.inkSoft, fontFamily: "'IBM Plex Mono', monospace" }}
            >
              Weekly availability
            </label>
          </div>
          ...
          >
            {loading ? "Saving…" : <>Save and continue <ArrowRight size={16} /></>}
          </button>
        </form>
      </div>
    </div>
